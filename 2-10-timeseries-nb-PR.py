...
# %%
# à vous
#ça passe de NaN à NaT (not a number --> not a time)
...
plt.title('High (red) and Low (blue)')
plt.show()
...
# %%
# à vous
moy = df2.resample('1W').mean()
moy.plot()
plt.show()

moy.shape
#je n'ai plu que 79 valeurs, il y en a moins
...
# %%
# à vous
df1 = df.rolling(window = pd.Timedelta(365, 'D'), center=True).count()
