...
```{code-cell} ipython3
# à vous
d1 = df1.set_index('name')
d2 = df2.set_index('name')
d3 = df3.set_index('name')
df4 = pd.merge(df1, df2, on="name")
df5 = pd.concat([df4.set_index('name'),df3.set_index('name')])
...
m4 = pd.merge(m1,m2, left_on='nom', right_on='last_name')
m4 = m4.drop(['first_name','last_name'], axis=1)

m3 = m3.set_axis(['prenom','nom','age','grade','height','sex'], axis = 1)
m5 = pd.concat([m3,m4])
m5
```
